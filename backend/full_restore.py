#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""完整恢复所有文件"""
import shutil
from pathlib import Path
import os

BASE_DIR = Path(__file__).parent
APP_DIR = BASE_DIR / "app"
BACKUP_DIR = BASE_DIR / "backup"

def restore_all_files():
    """恢复所有文件"""
    print("=" * 70)
    print("Complete Restoration - Restoring All Files")
    print("=" * 70)
    
    restored = 0
    
    # 方法1: 从 backup/api_v1 恢复 API 文件
    api_backup = BACKUP_DIR / "api_v1"
    if api_backup.exists():
        for backup_file in api_backup.glob("*.py"):
            target = APP_DIR / "api" / "v1" / backup_file.name
            if target.exists():
                shutil.copy2(backup_file, target)
                restored += 1
                print(f"[OK] Restored: {target.name}")
    
    # 方法2: 从 backup/service_restructure 恢复服务文件
    service_backup = BACKUP_DIR / "service_restructure" / "services"
    if service_backup.exists():
        for backup_file in service_backup.rglob("*.py"):
            rel_path = backup_file.relative_to(service_backup)
            target = APP_DIR / "services" / rel_path
            target.parent.mkdir(parents=True, exist_ok=True)
            if target.exists():
                shutil.copy2(backup_file, target)
                restored += 1
                print(f"[OK] Restored: {target.relative_to(BASE_DIR)}")
    
    # 方法3: 从 backup/structure_optimization 恢复重命名的文件
    rename_backup = BACKUP_DIR / "structure_optimization"
    if rename_backup.exists():
        for backup_file in rename_backup.rglob("*.py"):
            rel_path = backup_file.relative_to(rename_backup)
            # 尝试恢复可能的目标位置
            for search_path in [APP_DIR / "api" / "v1", APP_DIR / "models"]:
                target = search_path / rel_path
                if target.exists():
                    shutil.copy2(backup_file, target)
                    restored += 1
                    print(f"[OK] Restored: {target.relative_to(BASE_DIR)}")
                    break
    
    return restored

def verify_and_fix():
    """验证并修复"""
    print("\n" + "=" * 70)
    print("Verifying and Fixing")
    print("=" * 70)
    
    import ast
    errors = []
    
    for filepath in APP_DIR.rglob("*.py"):
        if "__pycache__" in str(filepath):
            continue
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                ast.parse(f.read())
        except SyntaxError as e:
            errors.append((str(filepath.relative_to(BASE_DIR)), str(e)))
    
    return errors

def main():
    """主函数"""
    print("\n" + "=" * 70)
    print("Project Structure Fix - Complete Restoration")
    print("=" * 70)
    
    # 恢复文件
    restored = restore_all_files()
    print(f"\n[OK] Restored {restored} files from backup")
    
    # 验证
    errors = verify_and_fix()
    
    if errors:
        print(f"\n[ERROR] Found {len(errors)} syntax errors:")
        for filepath, error in errors[:10]:
            print(f"  {filepath}: {error}")
        
        # 尝试删除所有有问题的文件并提示用户
        print("\n[INFO] Consider deleting the following files and recreating them:")
        for filepath, _ in errors[:5]:
            print(f"  - {filepath}")
        
        return False
    else:
        print("\n[OK] All syntax errors fixed!")
        return True

if __name__ == "__main__":
    success = main()
    exit(0 if success else 1)
