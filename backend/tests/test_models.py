...
class TestModels:
    ...
    def test_create_model_duplicate(self, client, auth_headers):
        """创建同名模型应失败"""
        client.post("/api/v1/models/", json=self.CREATE_PAYLOAD, headers=auth_headers)
        resp = client.post("/api/v1/models/", json=self.CREATE_PAYLOAD, headers=auth_headers)
    ...
    def test_list_models(self, client, auth_headers):
        ...
        resp = client.get("/api/v1/models/?page=1&page_size=10", headers=auth_headers)
    ...
    def test_get_model_not_found(self, client, auth_headers):
        """获取不存在的模型"""
        resp = client.get("/api/v1/models/nonexistent-id", headers=auth_headers)
    ...
    def test_test_connection_skip(self, client, auth_headers):
    ...
    def test_pagination(self, client, auth_headers):
        """分页参数"""
        resp = client.get("/api/v1/models/?page=1&page_size=5", headers=auth_headers)
        assert resp.status_code == 200
        data = resp.json()
        assert data["page"] == 1
        assert data["page_size"] == 5

    def test_create_model_with_default(self, client, auth_headers):
        """创建默认模型"""
