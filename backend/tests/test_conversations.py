...
class TestConversations:
    ...
    async def test_create_conversation(self, client, auth_headers):
        ...
        data = await self._create_conv(client, auth_headers, agent_id)
        assert data["title"] == "测试对话"
        assert data["agent_id"] == agent_id
        assert "id" in data
        assert data["status"] == "active"

    async def test_create_conversation_missing_title(self, client, auth_headers):
        """创建缺少标题的对话应失败"""
        agent_id = await self._create_agent_id(client, auth_headers)
        resp = await client.post(
    ...
    async def test_list_conversations(self, client, auth_headers):
        """对话列表"""
        agent_id = await self._create_agent_id(client, auth_headers)
        await self._create_conv(client, auth_headers, agent_id)
        resp = await client.get("/api/v1/conversations/?page=1&page_size=10", headers=auth_headers)
    ...
    async def test_get_conversation(self, client, auth_headers):
        """获取对话详情"""
        agent_id = await self._create_agent_id(client, auth_headers)
        conv = await self._create_conv(client, auth_headers, agent_id)
        resp = await client.get(f"/api/v1/conversations/{conv['id']}", headers=auth_headers)
        assert resp.status_code == 200
    ...
    async def test_update_conversation_title(self, client, auth_headers):
        """更新对话标题"""
        agent_id = await self._create_agent_id(client, auth_headers)
        conv = await self._create_conv(client, auth_headers, agent_id)
    ...
    async def test_update_status_archived(self, client, auth_headers):
        """归档对话"""
        agent_id = await self._create_agent_id(client, auth_headers)
        conv = await self._create_conv(client, auth_headers, agent_id)
    ...
    async def test_search_conversations(self, client, auth_headers):
        """搜索对话"""
        agent_id = await self._create_agent_id(client, auth_headers)
        await self._create_conv(client, auth_headers, agent_id, title="特殊搜索词ABC123")
    ...
    async def test_send_message_invalid_role(self, client, auth_headers):
        """发送消息时使用无效角色应失败"""
        agent_id = await self._create_agent_id(client, auth_headers)
        conv = await self._create_conv(client, auth_headers, agent_id)
        resp = await client.post(
    ...
    async def test_message_with_tokens(self, client, auth_headers):
        """发送带 token 计数的消息"""
        agent_id = await self._create_agent_id(client, auth_headers)
        conv = await self._create_conv(client, auth_headers, agent_id)
        resp = await client.post(
