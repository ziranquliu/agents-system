...
@pytest.mark.asyncio
async def test_cache_set(cache_manager):
    """测试缓存写入"""
    mock_redis = AsyncMock()
...
@pytest.mark.asyncio
async def test_invalidate_pattern(cache_manager):
    """测试批量失效缓存"""
    mock_redis = AsyncMock()
...
@pytest.mark.asyncio
async def test_cache_with_none_value(cache_manager):
    """测试None值的处理"""
    mock_redis = AsyncMock()
    mock_redis.get = AsyncMock(return_value=None)
    cache_manager._redis = mock_redis
...
@pytest.mark.asyncio
async def test_json_serialization(cache_manager):
    """测试JSON序列化"""
    mock_redis = AsyncMock()
