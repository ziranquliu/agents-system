...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    try:
        ...
        logger.info(r" Server started: {settings.PROJECT_NAME} v{settings.PROJECT_VERSION}")
    except Exception as e:
        logger.warning(r" Startup warning: {e}")
    # 启动全局定时调度器
    try:
        from app.core.scheduler import start_scheduler
        sched = start_scheduler()
        logger.info(r" Scheduler started with {len(sched.get_jobs())} jobs")
    except Exception as e:
        logger.warning(r" Scheduler start failed (non-fatal): {e}")
    yield
    from app.core.scheduler import stop_scheduler
    stop_scheduler()
    from app.db.session import close_db_connections
    await close_db_connections()
    logger.info(r""[INFO] Server shutdown complete"")
